...
function M.MapLspKeys(ev)
	...
	keymap.set({"n", "v"}, "<leader>ha", vim.lsp.buf.code_action, vim.tbl_extend("force", opts, {desc="See available code actions"}))
	keymap.set("n", "<leader>hk", function() vim.lsp.buf.hover({border = 'rounded',}) end, vim.tbl_extend("force", opts, {desc="Show documentation"}))
	keymap.set("n", "<leader>hH", "<cmd>Telescope diagnostics bufnr=0<CR>", vim.tbl_extend("force", opts, {desc="Show buffer diagnostics"}))
	keymap.set("n", "<leader>hh", function() vim.diagnostic.open_float(nil, {border = "rounded",scope = "line",focusable = false,})
	...
	keymap.set("n", "<leader>hx", helper.toggle_lsp, { desc = "Activate/Deactivate LSP" })
